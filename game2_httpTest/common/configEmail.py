...
class Email:
    '''
    初始化
    读取配置文件config.ini文件里的[Email]数据
    读取receiver值，按"/"分割，存入列表里
    定义date：当前时间
    '''
    def __init__(self):
        ...
        self.subject = "接口测试报告" + " " + self.date
        self.log = MyLog.get_log()
        self.logger = self.log.get_logger()
        self.msg = MIMEMultipart('related')
        '''
        MIMEMultipart的目录结构如下
                    Message
            +- MIMEBase
               +- MIMEMultipart
               +- MIMENonMultipart
